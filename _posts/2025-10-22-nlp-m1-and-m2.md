...
One key idea is that natural language has ample *syntactic ambiguity* - the same set of words can have different meanings depending on the context. Certain phrases may have intended connotations and interpretations that are more preferred to others.

### What Fields are Involved in NLP?

NLP is within the realm of **artificial intelligence (AI)**, where a significant portion of AI is **machine learning**. *Linguistics* and *speech* also contribute to NLP.

NLP is heavily reliant on **probability and statistics**. NLP may involve classification - for example, what is the probability that a movie review is positive?  

$$P(positive\_review | word_1, word_2, ..., word_n)$$
...
Much of NLP is also dedicated to **deep learning** - a class of machine learning algorithms that use chains of differentiable code modules to learn from data a function that maps inputs to outputs. In simpler terms, deep learning is a subfield of machine learning that uses *deep neural networks* - neural networks with many hidden layers - to learn patterns from data. Deep learning may be applied within supervised, unsupervised, and reinforcement learning.
...
$$ Random~Variable: X \textasciitilde N(0, 1) $$
$$ Observation: x = 0.5 $$
...
The **Naïve Bayes Assumption** assumes that effects are independent, enabling this calculation. 
